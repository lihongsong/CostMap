...
NSString *const HJZlibErrorDomain = @"se.bitba.hjZlibErrorDomain";
NSString *const HJZlibErrorInfoKey = @"zerror";

@implementation NSData (HJZlib)

static void *hj_zlibOpen()
{
    static void *libz;
...
- (NSData *)hj_dataByInflatingWithError:(NSError *__autoreleasing *)error
{
    ...
    [self hj_inflate:^(NSData *toAppend) {
...
- (NSData *)hj_dataByDeflatingWithError:(NSError *__autoreleasing *)error
{
    if (![self length]) return [self copy];
    NSMutableData *outData = [NSMutableData data];
    [self hj_deflate:^(NSData *toAppend) {
        [outData appendData:toAppend];
    }
               error:error];
    return outData;
...
- (BOOL)hj_inflate:(void (^)(NSData *))processBlock
             error:(NSError *__autoreleasing *)error
{
    void *libz = hj_zlibOpen();
    ...
    if (ret != Z_OK) {
        if (error) *error = [NSError errorWithDomain:HJZlibErrorDomain
                                                code:HJZlibErrorCodeInflationError
                                            userInfo:@{HJZlibErrorInfoKey : @(ret)}];
        return NO;
    }
...
// Adapted from http://www.zlib.net/zpipe.c
- (BOOL)hj_deflate:(void (^)(NSData *))processBlock
             error:(NSError *__autoreleasing *)error
{
    void *libz = hj_zlibOpen();
    //    int (*deflateInit2_)(z_streamp, int, int, int, int, int, const char *, int) =
    ...
    if (ret != Z_OK) {
        if (error) *error = [NSError errorWithDomain:HJZlibErrorDomain
                                                code:HJZlibErrorCodeDeflationError
                                            userInfo:@{HJZlibErrorInfoKey : @(ret)}];
        return NO;
    }
    Bytef *source = (Bytef *)[self bytes]; // yay
...
- (BOOL)hj_writeDeflatedToFile:(NSString *)path
                         error:(NSError *__autoreleasing *)error
{
    NSFileHandle *f = hj_createOrOpenFileAtPath(path, error);
    if (!f) return NO;
    BOOL success = YES;
    if ([self length]) {
        success = [self hj_deflate:
                   ^(NSData *toAppend) {
...
static NSFileHandle *hj_createOrOpenFileAtPath(NSString *path, NSError *__autoreleasing *error)
{
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
        ...
        if (!success) {
            if (error) *error = [NSError errorWithDomain:HJZlibErrorDomain
                                                    code:HJZlibErrorCodeCouldNotCreateFileError
                                                userInfo:nil];
            return nil;
        }
    }
