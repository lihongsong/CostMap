...
BOOL hj_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void hj_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void hj_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
@implementation NSObject (HJRuntime)


+ (void)hj_swizzleMethod:(SEL)originalMethod withMethod:(SEL)newMethod
{
    hj_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)hj_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    hj_method_append(self.class, klass, newMethod);
}

+ (void)hj_replaceMethod:(SEL)method fromClass:(Class)klass
{
    hj_method_replace(self.class, klass, method);
}

- (BOOL)hj_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class hj_instancesRespondToSelector:selector untilClass:stopClass];
}
...
{
    return [self.superclass hj_instancesRespondToSelector:selector untilClass:stopClass];
}
