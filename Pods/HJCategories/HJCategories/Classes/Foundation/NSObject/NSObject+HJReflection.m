...
@implementation NSObject (HJReflection)
...
{
    return [[self class] hj_propertyKeys];
}
+ (NSArray *)hj_propertyKeys {
...
{
    return [[self class] hj_propertiesInfo];
}
/**
 *  属性列表与属性的各种信息
 */
+ (NSArray *)hj_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self hj_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)hj_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] hj_propertiesInfo];
...
-(NSArray*)hj_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] hj_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] hj_decodeType:returnType];
        NSString *encodeString = [[self class] hj_decodeType:encoding];
...
-(NSDictionary *)hj_protocolList{
    return [[self class]hj_protocolList];
}
+ (NSDictionary *)hj_protocolList
{
...
+ (NSArray *)hj_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] hj_decodeType:ivar_getTypeEncoding(ivars[i])];
        NSString *name = [NSString stringWithCString:ivar_getName(ivars[i]) encoding:NSUTF8StringEncoding];
        NSString *ivarDescription = [NSString stringWithFormat:@"%@ %@", type, name];
        [result addObject:ivarDescription];
    }
    free(ivars);
    return result.count ? [result mutableCopy] : nil;
}
...
#pragma mark -- help
+ (NSDictionary *)hj_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)hj_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString hj_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
