...
@implementation UIDevice (HJHardware)
...
- (Hardware)hj_hardware {
    NSString *hardware = [self hj_hardwareString];
...
- (NSString *)hj_hardwareDescription {
    NSString *hardware = [self hj_hardwareString];
...
- (NSString *)hj_hardwareSimpleDescription
{
    NSString *hardware = [self hj_hardwareString];
    if ([hardware isEqualToString:@"iPhone1,1"])    return @"iPhone 2G";
    if ([hardware isEqualToString:@"iPhone1,2"])    return @"iPhone 3G";
...
- (float)hj_hardwareNumber:(Hardware)hardware {
    switch (hardware) {
...
- (BOOL)hj_isCurrentDeviceHardwareBetterThan:(Hardware)hardware {
    float otherHardware = [self hj_hardwareNumber:hardware];
    float currentHardware = [self hj_hardwareNumber:[self hj_hardware]];
    return currentHardware >= otherHardware;
}

- (CGSize)hj_backCameraStillImageResolutionInPixels
{
    switch ([self hj_hardware]) {
        case IPHONE_2G:
        case IPHONE_3G:
        ...
        default:
            ...
            NSLog(@"Your device is: %@", [self hj_hardwareDescription]);
            break;
    }
    return CGSizeZero;
}

- (BOOL)hj_isIphoneWith4inchDisplay
{
...
+ (NSUInteger)hj_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)hj_cpuFrequency {
    return [self hj_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)hj_busFrequency {
    return [self hj_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)hj_ramSize {
    return [self hj_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)hj_cpuNumber {
    return [self hj_getSysInfo:HW_NCPU];
}


+ (NSUInteger)hj_totalMemoryBytes
{
    return [self hj_getSysInfo:HW_PHYSMEM];
}

+ (NSUInteger)hj_freeMemoryBytes
{
...
+ (long long)hj_freeDiskSpaceBytes
{
    struct statfs buf;
    long long freespace;
    freespace = 0;
...
+ (long long)hj_totalDiskSpaceBytes
{
    struct statfs buf;
