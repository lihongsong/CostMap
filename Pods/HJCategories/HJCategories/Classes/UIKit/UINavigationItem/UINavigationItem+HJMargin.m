...
@implementation UINavigationItem (HJMargin)
+ (void)load
{
    // left
...
- (UIBarButtonItem *)spacerForItem:(UIBarButtonItem *)item withMargin:(CGFloat)margin
{
    ...
    spacer.width = margin - [self.class hj_systemMargin];
    if (!item.customView) {
...
{
    return [self spacerForItem:item withMargin:self.hj_leftMargin];
}

- (UIBarButtonItem *)rightSpacerForItem:(UIBarButtonItem *)item
{
    return [self spacerForItem:item withMargin:self.hj_rightMargin];
}


#pragma mark - Margin

- (CGFloat)hj_leftMargin
{
    NSNumber *value = objc_getAssociatedObject(self, @selector(hj_leftMargin));
    return value ? value.floatValue : [self.class hj_systemMargin];
}

- (void)setHj_leftMargin:(CGFloat)leftMargin
{
    objc_setAssociatedObject(self, @selector(hj_leftMargin), @(leftMargin), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    self.leftBarButtonItems = self.leftBarButtonItems;
}

- (CGFloat)hj_rightMargin
{
    NSNumber *value = objc_getAssociatedObject(self, @selector(hj_rightMargin));
    return value ? value.floatValue : [self.class hj_systemMargin];
}

- (void)setHj_rightMargin:(CGFloat)rightMargin
{
    objc_setAssociatedObject(self, @selector(hj_rightMargin), @(rightMargin), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
