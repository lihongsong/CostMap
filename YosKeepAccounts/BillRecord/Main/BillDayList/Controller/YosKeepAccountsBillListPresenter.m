#import "YosKeepAccountsBillListPresenter.h"
...
@interface YosKeepAccountsBillListPresenter ()<HJMediatorTargetInstance, YosKeepAccountsDaySelectedSceneDelegate>
...
@end
@implementation YosKeepAccountsBillListPresenter
+ (id)targetInstance {
    return [YosKeepAccountsBillListPresenter instance];
}
...
- (void)setUpUI {
    ...
    self.tableScene.tableType = YosKeepAccountsBillTableTypeDay;
    WEAK_SELF
    self.tableScene.deleteAction = ^(UITableViewCellEditingStyle editingStyle, YosKeepAccountsBillEntity * _Nonnull model) {
        ...
        NSArray *pieData = [YosKeepAccountsBillEntity templateBillArrayWithBills:tempArray];
...
- (void)requestData {
    ...
    [[YosKeepAccountsSQLManager share] searchData:kSQLTableName
                                 ...
                                 month:month
                                   day:day
                                result:^(NSMutableArray<YosKeepAccountsBillEntity *> *result, NSError *error) {
                                    STRONG_SELF
                                    NSArray *pieData = [YosKeepAccountsBillEntity templateBillArrayWithBills:result];
                                    self.pieScene.entitys = pieData;
