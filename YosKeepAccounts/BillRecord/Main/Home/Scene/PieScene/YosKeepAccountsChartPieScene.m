...
- (void)setEntitys:(NSArray<YosKeepAccountsBillEntity *> *)entitys {
    _entitys = entitys;
    __block double sum = 0.0f;
    [self.entitys enumerateObjectsUsingBlock:^(YosKeepAccountsBillEntity * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        sum += [obj.s_money doubleValue];
    }];
    NSMutableArray *values = [[NSMutableArray alloc] init];
    for (int i = 0; i < entitys.count; i++) {
        YosKeepAccountsBillEntity *model = entitys[i];
    ...
    dataSet.colors = [YosKeepAccountsBillTool allBillTypesColor];
...
- (void)setUp {
    ...
    parties = [YosKeepAccountsBillTool allBillTypesName];
