...
@implementation YosKeepAccountsProgressScene
...
- (void)initArrow:(CGRect)frame{
    self.bgScene = [YosKeepAccountsControl yka_createSceneWithFrame:CGRectMake(frame.size.width/2.0 - 7, 0, 14, frame.size.width)];
    UIImageView *arrowImage = [YosKeepAccountsControl yka_createImageSceneFrame:CGRectMake(0, 15, 14, 31) imageName:@"home_bg_dial_arrow"];
    UIImageView *lightImage = [YosKeepAccountsControl yka_createImageSceneFrame:CGRectMake(-7, -10, 29, 29) imageName:@"home_Light"];
    [self addSubview: self.bgScene];
    [self.bgScene addSubview:arrowImage];
    [self.bgScene addSubview:lightImage];
}
...
{
    self.bgScene.transform = CGAffineTransformMakeRotation(-(M_PI * 3 / 4.0));
    ...
    if ((self.toValue * M_PI * 2 - M_PI/4.0) > M_PI) {
        [UIView animateWithDuration:1.2 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
                self.bgScene.transform = CGAffineTransformRotate(self.lastTransform, M_PI);
                self.lastTransform = CGAffineTransformRotate(self.lastTransform,M_PI);
        } completion:^(BOOL finished) {
            if ((self.toValue * M_PI * 2 - M_PI/4.0) > M_PI){
                [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
                    self.bgScene.transform = CGAffineTransformRotate(self.lastTransform, M_PI/4.0);
                }completion:^(BOOL finished) {
                }];
            }
        }];
        ...
        [UIView animateWithDuration:1.5 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
                self.bgScene.transform = CGAffineTransformRotate(self.lastTransform, self.toValue * (M_PI * 2) - M_PI/4.0);
        } completion:^(BOOL finished) {
        }];
    }
}
