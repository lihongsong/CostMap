...
@implementation YosKeepAccountsWangYiWave
{
    CADisplayLink *_link;
    CGFloat _offset;
    CAShapeLayer *_layer;
...
- (void)doAni
{
    ...
    for (CGFloat i = 0.0; i < _waveWidth; i ++) {
        CGFloat y = 1.1*_waveHeight*sinf(1.5*M_PI*i/_waveWidth + _offset*M_PI/_waveWidth) + _h;
    ...
    _layer.path = pathRef;
    _layer.fillColor = [UIColor colorFromHexCode:@"#f8e71c"].CGColor;
    CGPathRelease(pathRef);
    CGMutablePathRef pathRef2 = CGPathCreateMutable();
    CGFloat startY2 = _waveHeight*sinf(_offset*M_PI/_waveWidth + M_PI/4);
    CGPathMoveToPoint(pathRef2, NULL, 0, startY2);
    for (CGFloat i = 0.0; i < _waveWidth; i ++) {
        CGFloat y = _waveHeight*sinf(1.5*M_PI*i/_waveWidth + 3*_offset*M_PI/_waveWidth + M_PI/4) + _h;
    ...
    _layer2.fillColor = [UIColor colorFromHexCode:@"#f8e71c"].CGColor;
