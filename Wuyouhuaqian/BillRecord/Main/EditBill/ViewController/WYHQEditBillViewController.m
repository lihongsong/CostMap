...
- (void)viewDidLoad {
    ...
    [WYHQEditBillToolBar showEditBillToolBarOnSuperVC:self classify:@"医疗" selectedTimeHandler:^(NSString * _Nonnull timeStr) {
        ...
        STRONG_SELF
        self.classifyImageView.image = [UIImage imageNamed:[WYHQBillModel getTypePressedImage:typeStr]];
    }];
...
- (void)updateUI {
    if (self.billModel) {
        self.classifyImageView.image = [UIImage imageNamed:[WYHQBillModel getTypePressedImage:self.billModel.s_type_name]];
