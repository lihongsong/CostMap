...
@interface CLCustomDatePickerView () <UIPickerViewDataSource, UIPickerViewDelegate> {
...
/** 显示类型 */
@property(nonatomic, assign) CLCustomDatePickerMode showType;
/** 时间选择器的类型 */
...
/** 选中后的回调 */
@property(nonatomic, copy) CLCustomDateResultBlock resultBlock;
/** 取消选择的回调 */
@property(nonatomic, copy) CLCustomDateCancelBlock cancelBlock;

@end

@implementation CLCustomDatePickerView
...
+ (CLCustomDatePickerView *)showDatePickerWithTitle:(NSString *)title
    ...
    CLCustomDatePickerView *datePickerView = [[CLCustomDatePickerView alloc] initWithTitle:title dateType:type defaultSelValue:defaultSelValue minDate:minDate maxDate:maxDate isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (instancetype)initWithTitle:(NSString *)title
    ...
    if (self = [super init]) {
        ...
        if (defaultSelValue && defaultSelValue.length > 0) {
            ...
            if (!defaultSelDate) {
                CLCustomErrorLog(@"参数格式错误！参数 defaultSelValue 的正确格式是：%@", self.selectDateFormatter);
...
- (void)setupSelectDateFormatter:(CLCustomDatePickerMode)type {
    switch (type) {
        case CLCustomDatePickerModeTime: {
            self.selectDateFormatter = @"HH:mm";
            self.style = BRDatePickerStyleSystem;
            _datePickerMode = UIDatePickerModeTime;
        }
            break;
        case CLCustomDatePickerModeDate: {
            self.selectDateFormatter = @"yyyy-MM-dd";
            self.style = BRDatePickerStyleSystem;
            _datePickerMode = UIDatePickerModeDate;
        }
            break;
        case CLCustomDatePickerModeDateAndTime: {
            self.selectDateFormatter = @"yyyy-MM-dd HH:mm";
            self.style = BRDatePickerStyleSystem;
            _datePickerMode = UIDatePickerModeDateAndTime;
        }
            break;
        case CLCustomDatePickerModeCountDownTimer: {
            self.selectDateFormatter = @"HH:mm";
            self.style = BRDatePickerStyleSystem;
        ...
        case CLCustomDatePickerModeYMDHM: {
            self.selectDateFormatter = @"yyyy-MM-dd HH:mm:ss";
            self.style = BRDatePickerStyleCustom;
        }
            break;
        case CLCustomDatePickerModeMDHM: {
            self.selectDateFormatter = @"MM-dd HH:mm";
            self.style = BRDatePickerStyleCustom;
        }
            break;
        case CLCustomDatePickerModeYMD: {
            self.selectDateFormatter = @"yyyy-MM-dd";
            self.style = BRDatePickerStyleCustom;
        }
            break;
        case CLCustomDatePickerModeYM: {
            self.selectDateFormatter = @"yyyy-MM";
            self.style = BRDatePickerStyleCustom;
        }
            break;
        case CLCustomDatePickerModeY: {
            self.selectDateFormatter = @"yyyy";
            self.style = BRDatePickerStyleCustom;
        }
            break;
        case CLCustomDatePickerModeMD: {
            self.selectDateFormatter = @"MM-dd";
            self.style = BRDatePickerStyleCustom;
        }
            break;
        case CLCustomDatePickerModeHM: {
            self.selectDateFormatter = @"HH:mm";
            self.style = BRDatePickerStyleCustom;
        }
            break;
...
- (void)scrollToSelectDate {
    ...
    if (self.showType == CLCustomDatePickerModeYMDHM) {
        indexArr = @[@(_yearIndex), @(_monthIndex), @(_dayIndex), @(_hourIndex), @(_minuteIndex)];
    } else if (self.showType == CLCustomDatePickerModeMDHM) {
        indexArr = @[@(_monthIndex), @(_dayIndex), @(_hourIndex), @(_minuteIndex)];
    } else if (self.showType == CLCustomDatePickerModeYMD) {
        indexArr = @[@(_yearIndex), @(_monthIndex), @(_dayIndex)];
    } else if (self.showType == CLCustomDatePickerModeYM) {
        indexArr = @[@(_yearIndex), @(_monthIndex)];
    } else if (self.showType == CLCustomDatePickerModeY) {
        indexArr = @[@(_yearIndex)];
    } else if (self.showType == CLCustomDatePickerModeMD) {
        indexArr = @[@(_monthIndex), @(_dayIndex)];
    } else if (self.showType == CLCustomDatePickerModeHM) {
...
// 4.选中时回调的委托方法，在此方法中实现省*份和城*市间的联动
...
- (void)showWithAnimation:(BOOL)animation {
    ...
    if (animation) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            CGRect rect = self.alertView.frame;
            rect.origin.y -= kPickerHeight + kTopViewHeight + HJ_BottombarH;
            self.alertView.frame = rect;
        }];
    }
}
